...
\begin{figure}[ht]
  ...
  tree]{An expression tree representing a query to find which stars worked for
    which studios in 1996, example taken from~\cite[p.
...
Approximating the cardinality is however how trivial as it leads to the question
...
algorithm is built on the observation that the \sql{JOIN}s are independent ---
the $i$th \sql{JOIN} is independent of how the first $i-1$ relations were
